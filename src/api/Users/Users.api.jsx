...
export async function userLogin(data) {
  return new Promise((resolve, reject) => {
    http
      ...
      .catch((err) => reject(err?.response?.data?.detail));
  });
}
...
export async function updateSingleProfile(data) {
  return new Promise((resolve, reject) => {
    axiosPrivate
      .patch(
        `${URL.EDITPROFILE}/${data.orgusername}`,
        {
...
export async function updateSingleProfileImage(data) {
  const formDataObj = {};
  data.formDataBody.forEach((value, key) => (formDataObj[key] = value));
  console.log(formDataObj);

  return new Promise((resolve, reject) => {
    axiosPrivate
      .patch(
        `${URL.EDITPROFILE}/${data.username}`,
        {
          image: data.formDataBody.image,
        },
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
